...
def parse_xml_to_dict_old(element):
    """recursively loop through xml elements and extract information into a dictionary"""

    def _add_child_to_result(child_data, child_name, result):
        ...
        if child_name in result:
            if isinstance(result[child_name], list):
                ...
                result[child_name] = [result[child_name], child_data]
        else:
    ...
    if "name" in element.attrib:
        ...
        if len(element) == 0:
            return (
                {element.attrib["name"]: {"tag": element.tag, **result}}
                if result
                else None
            )
        ...
        return (
            {element.attrib["name"]: {"tag": element.tag, **result}} if result else None
        )
...
def compare_mandatory_attributes(a1: dict, a2: dict) -> Optional[bool]:
    ...
    if mandatory1 != mandatory2 or not any(m for m in [mandatory1, mandatory2]):
        # case 1: mandatories are either different or not set
        mandatory = None
    else:
        mandatory = True
    return mandatory
...
def main():
    ...
    if len(all_systems) == 1:
        ...
        raise ValueError(f"Cannot merge empty feature model and more than two should not exist, but got {all_systems=}")
    merged_dict = {system: {"attributes": attrs, "children": {}}}

    merged_child_dict = merge_children(children1, children2, merged_dict)
    for feature, values in merged_child_dict.items():
        merged_dict[feature]["children"].update(values["children"])
    merged_dict[system]["children"].update(merged_child_dict)
...
def merge_children(c1: dict, c2: dict, p: dict) -> dict:
    """compare and merge children_dicts (c1 and c2) and update parent_dict (p)"""
    result_dict= {}
    for feature in p.keys():
        result_dict.update({feature: {"children":{}}})
        all_occurences = set(list(c1) + list(c2))
        duplicate_children = set(c1).intersection(set(c2))
        unique_children = all_occurences.difference(duplicate_children)
        for child in duplicate_children:
            child1_attrs, child2_attrs = (
                get_feature_element(d, "attributes", child)
                for d in [c1, c2]
            )
            if child1_attrs == child2_attrs:
                ...
                child_mandatory = compare_mandatory_attributes(
                    child1_attrs, child2_attrs
                )
                # fill attrs
                attrs = {"tag": child_tag}
            ...
            # p[feature]["children"].update(
            #     {child: {"attributes": attrs, "children": {}}}
            # )
            result_dict[feature]["children"].update({child: {"attributes": attrs, "children": {}}})
        for child in unique_children:
            fm1_names, fm2_names = (
                find_and_filter_all_keys(c) for c in [c1, c2]
            )
